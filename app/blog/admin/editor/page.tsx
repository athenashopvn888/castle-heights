...
function BlogEditorInner() {
  ...
  const [author, setAuthor] = useState("Castle Heights Team");
  ...
  useEffect(() => {
    ...
    fetch(`${APPS_SCRIPT_URL}?action=blog&store=CHC01&admin=1`)
      .then((r) => r.json())
      .then((data) => {
        ...
        if (post) {
          ...
          setAuthor(post.author || "Castle Heights Team");
  ...
  async function handleSave() {
    ...
    const payload = editId
      ...
      : { action: "create", title, content, author, published, store: "CHC01" };
